...
class Model(nn.Module):
    ...
    def __init__(self, backbone: BackboneBase, num_classes: int, pooler_mode: Pooler.Mode,
                 ...
                 anchor_smooth_l1_loss_beta: Optional[float] = None, proposal_smooth_l1_loss_beta: Optional[float] = None,need_profile:bool=False):
        super().__init__()
        ...
        # print(len(children))
        self.steps=[nn.Sequential(*children[:4]),children[4],children[5],children[6]]
        self.need_profile=need_profile
    ...
    def forward(self, image_batch: Tensor,
                gt_bboxes_batch: Tensor = None, gt_classes_batch: Tensor = None,image_batch_shape=None) -> Union[Tuple[Tensor, Tensor, Tensor, Tensor],
        ...
        if self.need_profile:
            ...
            for i in range(len(self.steps)):
                ...
                features=self.steps[i](features)
                ...
                inter_results.append(features)
            # print(torch.sum(torch.abs(features-self.features(image_batch))))
        else:
        ...
        # print(image_width, image_height,anchor_bboxes.shape)
        # print(anchor_bboxes[0,:10])
        if self.training:
            ...
            # print(anchor_objectnesses.shape,anchor_transformers.shape)
            # print(anchor_objectnesses[0,:10],anchor_transformers[0,:10])
            ...
            # print(proposal_bboxes.shape)
            # print(proposal_bboxes[0,:10])
