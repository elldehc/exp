...
class Actor(nn.Module):
    def __init__(self):
        ...
        self.fc_cnode=nn.Linear(128*3,10)
...
class Critic(nn.Module):
    def __init__(self):
        super().__init__()
        self.relu=nn.ReLU()
        self.lstm=nn.LSTM(EDGE_CLUSTER_NUM*(EDGE_NODE_NUM+CLOUD_NODE_NUM*CLOUD_CLUSTER_NUM)*4,EDGE_CLUSTER_NUM*(EDGE_NODE_NUM+CLOUD_NODE_NUM*CLOUD_CLUSTER_NUM)*128,1,batch_first=True)
        self.fc1=nn.Linear(128*3,128)
        self.fc2=nn.Linear(125,128)
        self.fc3=nn.Linear(256*EDGE_CLUSTER_NUM*(EDGE_NODE_NUM+CLOUD_NODE_NUM*CLOUD_CLUSTER_NUM),EDGE_CLUSTER_NUM*(EDGE_NODE_NUM+CLOUD_NODE_NUM*CLOUD_CLUSTER_NUM))
    ...
    def forward(self,res,pref,y_res,y_fr,y_estep,y_enode,y_ccluster,y_cnode):
        ...
        y1,_=self.lstm(res)
        y1=y1[:,-1,:]
        y1=torch.reshape(y1,[-1,EDGE_CLUSTER_NUM,EDGE_NODE_NUM+CLOUD_NODE_NUM*CLOUD_CLUSTER_NUM,128])
        ...
        y4=self.fc3(torch.reshape(y4,[-1,EDGE_CLUSTER_NUM*(EDGE_NODE_NUM+CLOUD_NODE_NUM*CLOUD_CLUSTER_NUM)*256]))
